...
const NodeStatusBoard = ({
    node, close
}: { node: PkgNodeInfo, close: () => void }) => {


    const [nodeLogs, setNodeLogs] = useState<string[]>([]);

    const fetchLogs = useCallback(() => {
        httpGet(`/ros/node/${node.pkg}/${node.node}`).then((data: { logs: string }) => {
            if (data.logs)
    ...
    const getLogLevel = (log: string) => {
        ...
        return <TextRibon color={color} size="sm" textColor={textColor}>{message}</TextRibon>
    }
    ...
    return <VStack style={{
        ...
        <HStack style={{
            ...
            <TextRibon color="#4AA785" size="md" textColor="#eeeeee">{node.pkg}</TextRibon>
            <TextRibon color="#1C1C1C66" size="md" textColor="#eeeeee">{node.node}</TextRibon>
        ...
        <VStack style={{
            ...
            overflowY: 'auto',
            gap: 0,
            background: '#e7e9eB', borderRadius: '1rem',
            padding: '2rem'
        }}>
            {
                nodeLogs.length === 0 && <Container style={{
                    width: '100%',
                    height: '100%',
            ...
            {
                nodeLogs.map((log, index) => <HStack key={index} style={{
                    height: '2rem',
                    minHeight: '2rem',
                    justifyContent: 'flex-start',
                    alignItems: 'flex-start',
