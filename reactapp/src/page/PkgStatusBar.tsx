import { Button, Container, Grid, HStack, Switch, Text, VStack } from "@chakra-ui/react"
...
const NodeButton = (props: { node: PkgNodeInfo, checkVisible: boolean, checked: boolean, onChange: (b: boolean) => void, onClick: () => void }) => {
    ...
    >
        ...
        >
            ...
            <Text color="#1C1C1C" fontSize="2xl">{node.name}</Text>

            <div style={{ flexGrow: 1 }} />
            <HStack style={{ width: '100%', justifyContent: 'flex-end', alignItems: 'flex-end' }}>
                <Text fontSize="sm" color="#1C1C1C66"
                    noOfLines={[1, 2, 3]}
                    textAlign={'left'}
                >{node.abstract}</Text>
                {
                    checkVisible && (
                        <div onClick={(e) => e.stopPropagation()}>
                            <Switch size="lg" isChecked={checked}
                                onChange={(e) => {
                                    e.stopPropagation();
...
const PkgStatusBar = ({ onClickNode }: { onClickNode?: (node: PkgNodeInfo) => void }) => {
    ...
    useEffect(() => {
        rosSocket.subscribe('/pkg_monitoring', (data: { [key: string]: boolean }) => {
            setServiceStatus(data);
        });

        httpGet('/ros/node/list').then((data: PkgNodeInfo[]) => {
            if (data)
                setNodeList(data);
        });
    }, []);
    ...
    const killNode = (pkg: string, node: string) => {
        ...
        window.confirm(`Do you want to kill ${node} in ${pkg} ?`) &&
            httpDel(`/ros/node/${pkg}/${node}`).then((data: any) => {
    ...
    return (
        ...
        >{
                nodeList && nodeList.map((node, index) => (
                    ...
                    <NodeButton key={index} node={node} checkVisible={
