...
class RangerNode(Node):
    def __init__(self):
        ...
        self._action_robot_driver_forward = ActionServer(
            self,
            DriveDistance,
            "drive_distance_forward",
            self.action_robot_drive_forward_callback,
        )
    ...
    async def action_robot_drive_forward_callback(self, goal_handle: ServerGoalHandle):
        ...
        # 로봇의 local x 방향 unit vector 계산 (ROS에서는 기본적으로 x: forward, y: left)
        local_x_unit = (math.cos(start_yaw), math.sin(start_yaw))
        # 시작 위치 (global 좌표계)
        ...
        while rclpy.ok():
            ...
            # scalar projection: displacement vector와 local x unit vector의 내적 계산
            traveled_distance = dx * local_x_unit[0] + dy * local_x_unit[1]
            # 남은 거리 계산
            remaining_distance = distance - traveled_distance

            # 비례 속도 제어 (local x 방향으로 제어)
            cmd.linear.x = (
