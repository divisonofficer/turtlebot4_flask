...
class RosProtoConverter:

    def __init__(self):
        pass

    def rosPointToProtoPoint3D(self, msg: Point):
        return Point3D(x=msg.x, y=msg.y, z=msg.z)

    def rosQuaternionToProtoQuaternion(self, msg: Quaternion):
        return RobotQuaternionOrientation(x=msg.x, y=msg.y, z=msg.z, w=msg.w)

    def rosPoseToProtoPose3D(self, msg: Pose):
        quaternionAngle = self.rosQuaternionToProtoQuaternion(msg.orientation)
        eulerAngle = self.quaternionToEuler(quaternionAngle)

        return Pose3D(
            position=self.rosPointToProtoPoint3D(msg.position),
            orientation_euler=eulerAngle,
            orientation=quaternionAngle,
        )

    def quaternionToEuler(self, q: RobotQuaternionOrientation):
        w, x, y, z = (
            q.w,
            q.x,
            q.y,
        ...
        return RobotEuilerOrientation(roll=roll_x, pitch=pitch_y, yaw=yaw_z)
...
# class Pose3D:
#     def __init__(
#         self, position: Point3D, orientation: Union[EulerAngle, QuaternionAngle]
#     ):
#         self.position = position
#         if isinstance(orientation, EulerAngle):
...
#         id: int,
#         position: Point3D,
#         orientation: Union[EulerAngle, QuaternionAngle],
#     ):
#         self.id = id
#         self.position = position
#         if isinstance(orientation, EulerAngle):
